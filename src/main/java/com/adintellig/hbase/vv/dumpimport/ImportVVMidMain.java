...
public class ImportVVMidMain extends Configured implements Tool {
	static final Log LOG = LogFactory.getLog(ImportVVMidMain.class);
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(ImportVVMidMain.class);
		job.setMapperClass(ImportVVMidMapper.class);
	...
	public static void main(String[] args) throws IOException {
		...
		int res = 1;
		try {
			res = ToolRunner.run(conf, new ImportVVMidMain(), otherArgs);
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.exit(res);
